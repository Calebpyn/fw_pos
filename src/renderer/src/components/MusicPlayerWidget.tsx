...
function MusicPlayerWidget() {
    ...
    return (
        <div className="w-64 h-64 rounded-3xl text-white cursor-pointer transitions music-player-bg bg-cover bg-no-repeat bg-center">
            <div className="w-full flex h-full rounded-3xl bg-black bg-opacity-25 music-player-blur p-5 flex-col justify-start">
                <div className="w-full">
                    <span className="w-full flex justify-start text-10 text-white font-extralight mb-1">Sound System Plugin</span>
                </div>
                <div className="w-full flex justify-start items-end mt-2">
                    ...
                    <div className="ml-2 flex flex-col pb-5">
                        ...
                        <span className="overflow-x-hidden w-full text-8-c font-extralight">{testName.artist} - {testName.album}</span>
                    </div>
                </div>
                <div className="w-full">
                    <input type="range" className="w-full c-range-input" max={testName.lenght} min="0" defaultValue={currentTime} onChange={(e) => {
                    ...
                    <div className="w-full flex justify-center pt-3">
                        <div className="w-2/3 flex justify-between items-center">
                            ...
                            <div className="flex h-7 items-center" onClick={() => setIsPaused(!isPaused)}>
                                <div className={isPaused ? "play-button -mr-1 transitions" : "h-6 w-1 bg-white mr-1 transitions"}></div>
                                <div className={isPaused ? "w-0 transitions" : "h-6 w-1 bg-white transitions"}></div>
                            </div>
